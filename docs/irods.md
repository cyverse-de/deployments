# iRODS and Data Store Operations

This page covers the DE's connection to iRODS, how to verify it is working, how to
diagnose `data-info` connectivity failures, the iRODS CSI driver, the service account,
and file transfer troubleshooting.

---

## Overview

iRODS is the data store for all user files. The DE connects to it in several ways:

| Component | How it connects | Purpose |
|---|---|---|
| `data-info` | Jargon (iRODS Java API, direct TCP) | File browser, metadata, sharing |
...
| `info-typer` / `dewey` | AMQP events from iRODS | File type detection, indexing |
...
The service account password is stored in `irods_password` in the private inventory and
in the `configs` Kubernetes Secret.
...
The CSI driver uses a Helm-managed Secret (deployed by the `kubernetes_irods_csi_driver` role
into the `irods-csi-driver` namespace). The correct way to update it is to change the
...
The iRODS CSI driver mounts user home directories into VICE pods. If it is unhealthy, users'
files are not visible inside their running VICE apps.
...
If the listing fails or the directory appears empty when it shouldn't, the CSI driver mount
failed. Check the pod's volume status:
...
[batch-analyses-troubleshooting.md](batch-analyses-troubleshooting.md) §3 for diagnosing
when outputs don't appear after a job completes.
...
uploads/downloads triggered from the DE UI ("Save Outputs" button). It is separate from the
CSI driver mount.
...
Batch workflows use a generic PVC volume as the working directory. The `upload-files`
step transfers only files from this volume back to iRODS via `gocmd`. If the tool writes
to `/tmp` or any other path outside the working volume, those files are not transferred.
...
**Resolution:** Inform the user that only files written under `/data-store/` persist
after the session ends. There is no recovery path for files written elsewhere once the
pod is deleted.
