...
* **Update**: [groups](/services/groups.md) and [Importing Groups from Grouper](/playbooks/grouper-import.md) — behavior changes from the review-fix pass. `GET /` now answers 503 when the database ping fails, so the `/` health probes gate readiness on the database rather than always reporting 200; a Keycloak outage degrades lookups without marking the pod unready. The importer now exits nonzero when its own verification fails — a closure mismatch or members that failed to migrate — where it previously printed the mismatch and exited 0, so a scheduled run that detected wrong authorization data looked clean to automation; the informational lists (groups gone from Grouper, created natively) still affect only the report.
...
* **Update**: [Importing Groups from Grouper](/playbooks/grouper-import.md) — migrations `000057` and `000059` close two concurrency races in the effective-membership closure, both reproduced deterministically. The first: two writers rebuilding a shared ancestor collide on `group_effective_members_pkey`, one rolls back, and its membership change is lost — `000057` serializes them by locking the groups being rebuilt, in id order so intersecting ancestor sets cannot deadlock. The second is silent and `000057` does **not** fix it, because the two writers lock disjoint sets: one recomputes a group and its ancestors, which do not yet include a parent being attached concurrently; the other attaches it and recomputes the new parent from a snapshot missing the new member. Both commit, nothing recomputes again, and every permission granted to that parent fails to reach the member. The writers do share the group being attached, so `000059` locks it there. Verified by toggling each migration and re-running the interleaving: without them the parent holds only the pre-existing member, with them both.

* **Update**: [groups](/services/groups.md) — the subject search returns groups again. Grouper's returned teams and collaborator lists alongside users, tagged `source_id: "g:gsa"`, and the DE's sharing dialog recognizes a group by exactly that tag; returning only users made sharing data, apps, analyses, and tools with a team or collaborator list unreachable from the UI. The group half reuses the access-filtered listing, so it cannot surface a group the caller could not already see — verified against the running services, where an unrelated user finds neither of two private teams that their owner and the admin account both find. The `name` is now the short name where Grouper returned the full colon path; callers that strip the path get the same answer from either.

* **Update**: [groups](/services/groups.md) — joining is a distinct privilege again. Grouper spent three on the all-users subject and the DE used all three: `viewers` (discoverable), `readers` (members listable), and `optins` (self-join). Public communities carried read+optin and public teams carried view alone, so Grouper refused a self-join against a public team and the DE routed those through the join-request flow for an administrator to approve — verified against the running Grouper, which answers `success: false`. Gating on the public marker alone let any user add themselves to any public team they could see, bypassing that approval; `groups.joinable` (migration `000058`) records `optins` separately from `members_public`, which records `readers`. The two coincide in current data and mean different things, so deriving one from the other would be wrong the first time they diverge.

* **Update**: [groups](/services/groups.md) — records two rules a comprehensive review turned up. Group management requires the `admin` level, not `write` or `own`: the DE's precedence is own=0, write=1, admin=2, read=3, so `admin` sits *below* write and asking for write 403'd every co-admin the UI listed as one — verified against the running service, where a user granted `admin` on a team could not add a member. And `groups_admin_users` must contain `de_grouper`, which it now defaults to; it was `[]`, so everything worked locally only because the local inventory set it, while QA and production would have had apps 500 resolving communities and group-propagator receive withheld member lists. A withheld list now carries `"redacted": true` so a machine caller can tell it from an empty group — the propagator replaces iRODS ACLs from that endpoint, so treating a redaction as truth erased the group and logged it as an unremarkable "0 members".

* **Update**: [groups](/services/groups.md) — group listings are now access-filtered, restoring parity with Grouper's privilege-filtered group search. `GET /groups` had no authorization at all, so any user could enumerate every team and community, and every other user's personal collaborator lists, by name, description, and owner — on a production-sized dataset one page exposed 1,000 collaborator lists belonging to 999 different users. It also made the DE's team page reachable in a broken state, offering teams that 403 when opened, which is the "failed to get team details" users were seeing. The filter mirrors the read check exactly — a grant held directly or through a group, effective membership, or the public marker — so a group lists if and only if it opens; verified in both directions against the synthetic dataset. It is one set of EXISTS clauses in the listing query rather than a permission call per group, which would be unusable against production's 2,583; measured at about 13 ms.

* **Update**: [Importing Groups from Grouper](/playbooks/grouper-import.md) — migration `000056` lets a set of nested groups be deleted in one statement. The BEFORE DELETE trigger recomputes the closure of every containing group, and when several nested groups go at once — a cascade from `subjects` deletes them row by row — a container could already be gone, so recomputing it inserted closure rows for a group no longer in `groups` and the foreign key failed the whole statement. Containers that are themselves being deleted are now skipped, since their closure is cascading away regardless. Three groups was not enough to reproduce it; it took 2,593 with 165 nested edges, because the cascade order has to be unfavourable.
