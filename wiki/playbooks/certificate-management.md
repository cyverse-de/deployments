...
and what to do when a certificate has expired or is about to expire. For the initial
deployment of the HAProxy combined certificate, see
[Deploying TLS Certificates](/playbooks/deploy-tls-certs.md); for how cert-manager itself
is installed and configured, see [cert-manager](/infrastructure/cert-manager.md).

## Certificate inventory

In the CyVerse production deployment, [HAProxy](/infrastructure/haproxy.md) terminates TLS
for external traffic using a certificate from an external CA, then connects to Traefik
internally with `verify none`. Other deployments may expose Traefik directly, in which case
the cert-manager certificates become user-facing.
...
externally-issued certificate. The internal certs still need to be valid for Traefik to
serve traffic, but browsers never see them.
...
cert-manager will not renew a certificate that is still within its validity period unless
you force it. To trigger immediate renewal:
...
common causes are:
- The Route 53 IAM credentials in the `cert-manager-letsencrypt-aws-creds` Secret are
  invalid or expired.
...
Decode the values and verify they are valid AWS access key/secret key. If the keys have
been rotated in AWS IAM, update the Secret and delete any failed `CertificateRequest`
objects to restart issuance:
...
If a CA certificate expires, all leaf certificates signed by it also become invalid.
Renewing the CA is the same process as renewing any other cert-manager Certificate (delete
the Secret), but after CA renewal you must also force-renew all leaf certificates signed by
that CA:
...
This is disruptive — all services will briefly lose their certs while cert-manager
reissues. Schedule this during a maintenance window.
...
`[de_proxy]` inventory group via the `tls_certs_main.yml` playbook — see
[Deploying TLS Certificates](/playbooks/deploy-tls-certs.md).
...
1. **Identify which cert:** Check all certificates (step 1 above). The `READY=False` cert
   is the culprit.
...
4. **Verify:** Decode the new cert (step 1) and confirm the `Not After` date is in the
   future.
...
The most common cause is that cert-manager is running but the `CertificateRequest` is
stuck. Describe the request to find the error:
